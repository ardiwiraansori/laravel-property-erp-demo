@php
  $menuGroups = [
    [
      'label' => 'Master Data',
      'icon' => 'fa-database',
      'match' => 'erp.properties,erp.blocks,erp.units,erp.customers',
      'items' => [
...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3" id="sidenav-main">
  <div class="sidenav-header">
    ...
    <a class="navbar-brand m-0 d-flex align-items-center text-wrap" href="{{ route('dashboard') }}">
      ...
      <span class="ms-3 font-weight-bold">Property ERP<br><small class="text-xs text-secondary">Portfolio Demo</small></span>
    </a>
  </div>
  ...
  <div class="collapse navbar-collapse w-auto h-auto" id="sidenav-collapse-main">
    <ul class="navbar-nav">
      <li class="nav-item">
        <a class="nav-link {{ request()->routeIs('dashboard') ? 'active' : '' }}" href="{{ route('dashboard') }}">
          <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
            <i class="fas fa-tachometer-alt text-dark text-sm {{ request()->routeIs('dashboard') ? 'text-white' : '' }}"></i>
          </div>
      ...
      @foreach ($menuGroups as $index => $group)
        ...
        @endphp
        <li class="nav-item">
          <a class="nav-link {{ $isGroupActive ? 'active' : '' }}" data-bs-toggle="collapse" href="#{{ $collapseId }}" role="button" aria-expanded="{{ $isGroupActive ? 'true' : 'false' }}" aria-controls="{{ $collapseId }}">
            <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
              <i class="fas {{ $group['icon'] }} text-dark text-sm {{ $isGroupActive ? 'text-white' : '' }}"></i>
            </div>
          ...
          <div class="collapse {{ $isGroupActive ? 'show' : '' }}" id="{{ $collapseId }}">
            <ul class="nav ms-4 ps-3">
              @foreach ($group['items'] as $item)
                <li class="nav-item">
                  <a class="nav-link py-2 {{ request()->routeIs($item['route']) ? 'active' : '' }}" href="{{ route($item['route']) }}">
                    <span class="sidenav-mini-icon text-xs me-2"><i class="fas {{ $item['icon'] }}"></i></span>
    ...
    <div class="card card-background shadow-none card-background-mask-primary" id="sidenavCard">
      ...
      <div class="card-body text-start p-3 w-100">
        ...
        <div class="docs-info">
          ...
          <p class="text-xs font-weight-bold text-white mb-2">CRM, finance, document tracking, dan operation flow.</p>
          <a href="{{ route('erp.sales-pipeline') }}" class="btn btn-white btn-sm w-100 mb-0">View Pipeline</a>
        </div>
      </div>
    </div>
  </div>
